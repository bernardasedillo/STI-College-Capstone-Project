...
$confirmedReservationsSql = "SELECT 
    id,
    reservation_type,
    checkin_date,
    checkin_time,
...
// Query confirmed reservations with correct column names
$bookingSql = "SELECT 
...
if ($stmt) {
    ...
    if ($result) {
        while ($booking = $result->fetch_assoc()) {
            ...
            // Check if events_package exists (it's an ID for Event Package reservations)
            if (!empty($booking['events_package']) && is_numeric($booking['events_package'])) {
                $bookedPackageIds[] = (int)$booking['events_package'];
                $isFullyBooked = true; // Package booking = full event booking
                
                // Fetch package details to get the name with time info
                $pkgDetailSql = "SELECT name, duration_hours, venue FROM prices WHERE id = ?";
                $pkgStmt = $conn->prepare($pkgDetailSql);
                if ($pkgStmt) {
                    $pkgStmt->bind_param("i", $booking['events_package']);
                    $pkgStmt->execute();
                    $pkgResult = $pkgStmt->get_result();
                    if ($pkgRow = $pkgResult->fetch_assoc()) {
                        $itemName = $pkgRow['name'];
                        $durationHours = $pkgRow['duration_hours'];
                        $itemType = 'event_package';
                    }
                    $pkgStmt->close();
                }
            }
            // Check for resort_package (Resort reservations)
            elseif (!empty($booking['resort_package']) && is_numeric($booking['resort_package'])) {
                // Fetch resort package details
                $resortPkgSql = "SELECT name, duration_hours FROM prices WHERE id = ?";
                $resortStmt = $conn->prepare($resortPkgSql);
                if ($resortStmt) {
                    $resortStmt->bind_param("i", $booking['resort_package']);
                    $resortStmt->execute();
                    $resortResult = $resortStmt->get_result();
                    if ($resortRow = $resortResult->fetch_assoc()) {
                        $itemName = $resortRow['name'];
                        $durationHours = $resortRow['duration_hours'];
                        $itemType = 'resort_package';
                    }
                    $resortStmt->close();
                }
            }
            ...
            if ($itemName) {
                $existingBookings[] = [
                    ...
                    'booking_duration_hours' => $booking['duration_hours'],
...
// If event is fully booked (package booking exists), mark it
if ($isFullyBooked) {
...
// Fetch excess rates
$excessRateSql = "SELECT name, price FROM prices WHERE venue = 'Event' AND name LIKE '%Excess Rate%'";
